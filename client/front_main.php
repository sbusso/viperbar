...
function viperbar_get_mailchimp($options, $button_style) {
	...
	if (!$ViperBar_api->errorCode) {
		...
		foreach($merge_vars as $var) {
			if ($var['field_type'] == 'text' && $options['name_field_disable'] != 'on') {
		...
		$mailchimp_form = '<span id="viperbar_mailchimp_form" class="ViperBar_form">' . $inputs. '<input type="submit" value="' . $options['text_button'] . '" name="subscribe" id="ViperBar_submit" style="' . $button_style . '" />' . '</span>';
	}
